...
export const PlayerTable1 = ({onPlayersUpdate}) => {
    ...
    useEffect(() => {
        const fetchPlayers = async () => {
            try {
                ...
                const userResponse = await axios.get(`http://localhost:5001/api/v1/user/${username}`);
                const userId = userResponse.data.user_id;
    
                // Fetch players by user id
                const playerResponse = await axios.get(`http://localhost:5001/api/v1/players/${userId}`);
        ...
        fetchPlayers();
    }, ['http://localhost:5001', onPlayersUpdate]);
    ...
    const savePlayer = async (updatedPlayer) => {
        try {
            const response = await axios.put(
                `http://localhost:5001/api/v1/playerupdate/${updatedPlayer.player_id}`,
                updatedPlayer
            );
    ...
    const addPlayer = async (newPlayerData) => {
        try {
            ...
            const response = await axios.post('http://localhost:5001/api/v1/newplayerform', newPlayer);
    ...
    const deletePlayer = async (playerId) => {
        try {
            const response = await axios.delete(`http://localhost:5001/api/v1/playerdelete/${playerId}`); 
